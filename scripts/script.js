...
function createSalesProduct (idName, name, price, quantity) {
    ...
    .append(
        ...
        .append($('<div>', {
            ...
            .append($('<div>', {
                class: 'availability'
            }).append($('<span>', {id: 'quantityAvailable', text: quantity}))
    ...
    .append(
        ...
        .append($('<span>', {class: 'number'}).append($('<input>', {
            id: 'saleQuantity',
            type: 'number',
            value: 1,
    ...
    .append($('<div>', {class: 'add'}).append($('<button>', {id: idName, class: 'add', text: 'ADD', disabled: disable})))
...
$('.productList.sales').on('click', 'button.add', function () {
    ...
    let totalAmount = parseFloat(parseInt(quantity) * parseFloat(price)).toFixed(2)

    productItem = {id: id, name: name, quantity: quantity, price: price, totalAmount: totalAmount}
    ...
    if (!idExists) {
        ...
        $.each(cartItems, function(i, value) {
            createCartProduct(value.id, value.name, value.quantity, value.price, value.totalAmount)
        })
    } 
    else {
        ...
        let finalTotalAmount = parseFloat(parseFloat(oldAmount) + parseFloat(totalAmount)).toFixed(2)
...
$('.productList.sales').on('click', 'button', function () {
    ...
    let quantityAvailable = parseInt(productInstance.find('#quantityAvailable').text())
    ...
    if ($(this).hasClass('minus')) {
        if (quantityInputVal <= 1) { quantityInput.val(1) }
